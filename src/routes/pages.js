...
router.get('/games', async (req, res) => {
  try {
    ...
    if (provider) {
      ...
      sql += ` AND REGEXP_REPLACE(LOWER(COALESCE(provider, '')), '[^a-z0-9]', '', 'g') LIKE $${params.length}`;
    }
    if (category) {
...
router.get('/providers', async (req, res) => {
  try {
    ...
    const provImgR = await query('SELECT provider_name, image_url FROM provider_images WHERE is_active = TRUE ORDER BY sort_order, id');
    const normalizeProviderName = value => String(value || '').replace(/[^a-z0-9]/gi, '').toLowerCase();
    const activeProviderKeys = Object.keys(gameCounts).map(normalizeProviderName);
    const providers = provImgR.rows.map(r => r.provider_name).filter(name => {
      const normalized = normalizeProviderName(name);
      return activeProviderKeys.some(active => active.includes(normalized) || normalized.includes(active));
    });
